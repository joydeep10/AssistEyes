...
st.sidebar.write("Welcome to AssistEyes! A smart AI app designed to empower visually impaired individuals with real-time insights and assistance.")
st.sidebar.markdown("---")
...
llm1 = ChatGoogleGenerativeAI(model="gemini-1.5-flash", api_key=GOOGLE_API_KEY)
llm2 = ChatGoogleGenerativeAI(model="gemini-1.5-pro", api_key=GOOGLE_API_KEY)
...
if "need_assistance_triggered" not in st.session_state:
    st.session_state.need_assistance_triggered = False
...
def play_audio(translated_text, lang_code="en"):
...
# Streamlit App setup
st.title("Visual Guidance for the Impaired")
...
if describe_button and image_input:
    try:
        st.session_state.processed_image = process_image_input(image_input)
        st.image(st.session_state.processed_image, caption="Uploaded Image", use_container_width=True)

        # Scene Description (LLM1)
        message1 = HumanMessage(content=[
            {"type": "text", "text": "Describe the scene briefly in 100 to 200 words."},
            {"type": "image_url", "image_url": st.session_state.processed_image}
        ])
        response1 = llm1.invoke([message1])
        st.session_state.scene_description = re.sub(r"\*\*(.*?)\*\*", r"\1", response1.content)

        # Translate the scene description to the selected language
        translated_scene_description = translate_text(st.session_state.scene_description, selected_language_code)

        # Display translated text
        language_name = available_languages.get(language_code, "Unknown Language")
        st.write(f"### Scene Description in {language_name}:")
        st.write(translated_scene_description)

        # Play the translated scene description as audio
        play_audio(translated_scene_description, lang_code=selected_language_code)

        # Reset the "Need Assistance" trigger
        st.session_state.need_assistance_triggered = False
    except Exception as e:
        st.error(f"Error: {e}")

# Need Assistance Button
if st.session_state.scene_description and not st.session_state.need_assistance_triggered:
    if st.button("Need Assistance?"):
        try:
            st.session_state.need_assistance_triggered = True
            language_name = available_languages.get(language_code, "Unknown Language")
            # Personalized Assistance (LLM2)
            message2 = HumanMessage(content=[
                {"type": "text", "text": "You are an assistant for visually impaired individuals. Provide detailed context specific guidance based on the Image. use numbers instead of bullet points"},
                {"type": "image_url", "image_url": st.session_state.processed_image}
            ])
            response2 = llm2.invoke([message2])
            personalized_guidance = re.sub(r"\*\*(.*?)\*\*", r"\1", response2.content)

            # Translate the personalized guidance to the selected language
            translated_guidance = translate_text(personalized_guidance, selected_language_code)

            # Display translated text
            st.write(f"### Personalized Assistance in {language_name}:")
            st.write(translated_guidance)

            # Play the translated personalized guidance as audio
...
if ocr_button and image_input:
    try:
        ...
        if extracted_text:
            ...
            # Translate extracted text to the selected language
            translated_text = translate_text(extracted_text, selected_language_code)

            # Display and play only the translated text
            ...
            translated_message = translate_text(no_text_message, selected_language_code)

            # Display and play translated "no text" message
...
if detect_button and image_input:
    try:
        ...
        if detected_objects:
            ...
            # Translate the combined guidance for the selected language
            translated_guidance = translate_text(combined_guidance, selected_language_code)

            # Display the translated guidance
            ...
            # Display and play translated "no objects" message
            st.warning(translated_message)
            play_audio(translated_message, lang_code=selected_language_code)
    except Exception as e:
        st.error(f"Error: {e}")
